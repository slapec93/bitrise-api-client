...
describe('Util', () => {
    describe('timeout', () => {
        it('should cancel long running operation', async () => {
            const TTFB = 10000000;
            const longRunningPromise = new Promise(resolve => setTimeout(resolve, TTFB));
            const operationPromise = syncify(() => timeout(TTFB / 2, longRunningPromise));
        ...
        it('should let continue normal operations with return values', async () => {
            ...
            const TTFB = 10000;
            const longRunningPromise = new Promise(resolve => setTimeout(() => resolve(returnValue), TTFB));
            const operationPromise = syncify(() => timeout(TTFB * 2, longRunningPromise));
    ...
    describe('includeField', () => {
        it('should add and override properties to source object', () => {
            const actual = includeField({ test: 'test', prop1: 'test-prop' }, { test: 'test-new', prop2: 'test-prop' });
        ...
        it('should handle null values', () => {
            expect(includeField(null, { test: 'test' })).toEqual({ test: 'test' });
        });
    });
